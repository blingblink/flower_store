...
export default function SelectMenu({ label, options, selected, setSelected }) {
  // const options = [
  //   { id: 1, value: 'Hello', secondary: 'World' },
  // ]
          ...
          <div className="relative mt-2 w-full">
            <Listbox.Button className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
              <span className="inline-flex w-full truncate">
                <span className="truncate ml-3">{selected.value}</span>
                <span className="ml-2 truncate text-gray-500">{selected.secondary}</span>
              </span>
                        ...
                        <div className="flex">
                          <span
                            className={classNames(
                              selected ? 'font-semibold' : 'font-normal', 'truncate',
                              'ml-3',
                            )}
                          >
                            {option.value}
